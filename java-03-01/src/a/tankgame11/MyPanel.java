...
public class MyPanel extends JPanel implements KeyListener, Runnable
{
    ...
    Vector<Enemy> vecEnemy = new Vector<>();
    ...
    //定义一个存放Node对象的Vector，用来恢复敌人坦克坐标和方向
    Vector<Node> ns = new Vector<>();

    public MyPanel(int key)
    {
        if (key == 2)
        {
            File f = new File(Recorder.getRecordPath());
            if (f.exists())
            {
                ns = Recorder.getNodesAndEnemyRecord();
            }
            else
            {
        ...
        //将MyPanel对象的 vecEnemy的地址 设置给 Recorder 的 vecEnemy
        //指向同一块内存
        Recorder.setEms(vecEnemy);
        ...
        switch (key)
        {
            ...
            case 2: //继续上局游戏
            {
                for (int i = 0; i < ns.size(); ++i)
                {
                    ...
                    Enemy e = new Enemy(n.getX(), n.getY());

                    //将ems设置给vecEnemy
                    e.setEms(vecEnemy);

                    //设置方向
                    e.setDirect(n.getDirect());
...
//        }
    ...
    public void showInfo(Graphics g)
    {
        ...
        g.drawString("你累积击毁地方坦克", 1020, 30);
        drawTank(1020, 60, g, 0, 0);

        g.setColor(Color.BLACK);
        g.drawString("x" + Recorder.getAllEnemys(), 1080, 100);
        ...
        switch (em.getDirect())
        {
            ...
            case 2:  //坦克向上和向下的方向
            {
                if (s.getX() >= em.getX() && s.getX() <= em.getX() + 40 && s.getY() >= em.getY() && s.getY() <= em.getY() + 60)
                {
                    ...
                    //当我方击毁敌人时，就对数据Recorder.allEnemys++
                    if (em instanceof Enemy)
                    {
                        Recorder.addAllEnemys();
                    }
